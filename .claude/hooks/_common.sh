#!/usr/bin/env bash
#
# Shared helpers for the Stop quality gate (lib-checks.sh).
#
...
# credo, sobelow, test) on every one of those is wasteful, so the gate is
# fingerprint-gated: it hashes the source files it cares about and skips
...
# hook).
#
# Sourced after REPO_DIR is set. Sets up the toolchain env the gate shares with
# the other hooks.
...
# fingerprint_library — content hash of the source set that feeds
# compile/format/credo/test.
fingerprint_library() {
  ( cd "$REPO_DIR" && \
    find . \
      \( -path ./deps -o -path ./_build -o -path ./.git -o -name node_modules \) -prune -o \
